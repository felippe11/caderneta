...
import { Settings, Save, School as SchoolIcon, Calendar, Plus, Trash2, Clock, Users, Shield, LayoutGrid } from 'lucide-react';

interface AdminSettingsProps {
  school: School;
}

type SettingsTab = 'GENERAL' | 'ATTENDANCE' | 'ACCESS';
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex bg-white p-1.5 rounded-xl border border-slate-200 shadow-sm overflow-x-auto">
        <button
          ...
          className={`flex-1 flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg text-sm font-semibold transition-all whitespace-nowrap ${
            activeTab === 'GENERAL' 
            ? 'bg-slate-900 text-white shadow-md' 
            : 'text-slate-500 hover:bg-slate-50 hover:text-slate-700'
          }`}
        >
          <LayoutGrid size={18} /> Geral
        </button>
        <button
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="bg-slate-50 px-8 py-5 flex justify-end border-t border-slate-100">
              <button className="flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-xl hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10 font-bold active:scale-95">
                 <Save className="w-4 h-4" /> Salvar Alterações Gerais
              </button>
            </div>
          </div>
        </div>
      ...
      {activeTab === 'ATTENDANCE' && (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="bg-slate-50 px-8 py-5 flex justify-end border-t border-slate-100">
              <button className="flex items-center gap-2 bg-slate-900 text-white px-6 py-3 rounded-xl hover:bg-slate-800 transition-all shadow-lg shadow-slate-900/10 font-bold active:scale-95">
                 <Save className="w-4 h-4" /> Salvar Regras de Frequência
              </button>
            </div>
          </div>
        </div>
